...
import { MOCK_DONORS, recordDonation, getDonationHistory } from '../data/mockDonors';
...
export default function MockPaymentGateway() {
  ...
  const donationHistory = getDonationHistory().slice(0, 10);
  ...
  const handleDonate = async (donorId: number) => {
    ...
    try {
      ...
      // Record donation
      recordDonation(donorId, trackingId, amount);
      ...
      const journey = {
        id: trackingId,
        waypoints: journeyWaypoints.map(w => ({ ...w, status: w.id === 1 ? 'active' as const : 'pending' as const })),
        currentStage: 1,
  ...
  return (
    <div className="scrollable-page min-h-screen bg-gradient-to-br from-purple-900 via-gray-900 to-black text-white p-8">
        ...
        {MOCK_DONORS.map((donor, index) => (
          ...
          >
            ...
            >
              ...
              {processing === donor.id ? 'Processing...' : 'DONATE'}
            </button>

            {donor.lastDonation && (
              <div className="mt-3 p-2 bg-green-500/10 rounded-lg border border-green-500/30">
                <p className="text-xs text-green-400">
                  Last: ${donor.lastDonation.amount} •{' '}
                  {new Date(donor.lastDonation.timestamp).toLocaleTimeString()}
                </p>
              </div>
            )}

            <div className="mt-3 text-center text-sm text-gray-500">
              Total: ${donor.totalDonated} ({donor.donationCount} donations)
            </div>
          </motion.div>
        ))}
      </div>
        ...
        >
          ...
          <div className="space-y-2">
            {donationHistory.map((donation, i) => (
              <div key={i} className="p-3 bg-gray-800/50 rounded-lg flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${
                    donation.status === 'completed' ? 'bg-green-500' :
                    donation.status === 'processing' ? 'bg-yellow-500' :
                    'bg-gray-500'
                  }`} />
      ...
      <div className="mt-8 flex gap-4">
        <Link to="/" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm">← Map</Link>
        <Link to="/admin" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm">Admin</Link>
        <Link to="/sms" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm">SMS →</Link>
      </div>
    </div>
  );
}
