...
export function useGlobalJourneyProgression() {
  ...
  const progressJourney = useCallback((journeyId: string) => {
    ...
    if (newWaypoint && currentJourney.metadata.donorPhone) {
      sendJourneyNotification(
        journeyId,
        nextStage,
        currentJourney.metadata.donorPhone,
        {
  ...
  useEffect(() => {
    ...
    activeJourneys.forEach(journey => {
      if (!intervalsRef.current.has(journey.id)) {
        console.log(`▶️ Starting progression for new journey: ${journey.id} (Stage ${journey.currentStage})`);
