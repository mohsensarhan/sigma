...
export function GlobalSettingsProvider({ children }: { children: ReactNode }) {
  const [settings, setSettings] = useState<GlobalSettings>(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          stepDuration: parsed.stepDuration || DEFAULT_STEP_DURATION,
          errorLogs: parsed.errorLogs || [],
          activeJourneys: parsed.activeJourneys || [],
          completedJourneys: parsed.completedJourneys || []
        };
      }
    } catch (error) {
    ...
    console.log('📂 Initializing fresh settings');
    return {
  ...
  useEffect(() => {
    try {
      ...
      console.log(`💾 Saved to localStorage - active: ${settings.activeJourneys.length}, completed: ${settings.completedJourneys.length}`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log(`🔄 settingsRef updated - active: ${settings.activeJourneys.length}, completed: ${settings.completedJourneys.length}`);
  }, [settings]);
  ...
  // Journey Management
  const registerJourney = useCallback((journey: Journey) => {
    console.log(`🔵 registerJourney called for: ${journey.id}`);
    console.log(`🔵 BEFORE registration - settingsRef.current.activeJourneys.length: ${settingsRef.current.activeJourneys.length}`);

    setSettings(prev => {
      console.log(`🔵 INSIDE setSettings - prev.activeJourneys.length: ${prev.activeJourneys.length}`);
    ...
    console.log(`🔵 AFTER setSettings called - settingsRef.current.activeJourneys.length: ${settingsRef.current.activeJourneys.length}`);
  ...
  const getJourney = useCallback((id: string) => {
    return settingsRef.current.activeJourneys.find(j => j.id === id) ||
  ...
  const getAllActiveJourneys = useCallback(() => {
    ...
    console.log(`   - settingsRef.current.activeJourneys.length: ${settingsRef.current.activeJourneys.length}`);
